/**
 * 🛡️ ModerationAgent v10.1 – Autonomous AI Moderation System + B2B Webhook Feed
 ...
 * - B2B webhook delivery: sends moderation events to all subscribers
 */
...
class ModerationAgent extends BaseAgent {
  constructor(eventBus, deps) {
    ...
    // ---- Models (for subscription lookups) ----
  ...
  async init() {
    ...
    this.logger.info(`🛡️ ModerationAgent v10.1 ready (webhook: ${hasWebhook ? '✅' : '❌'})`);
  }
  ...
  // ---------- B2B Webhook Delivery ----------
  async _deliverToB2B(embed, guildId, actionType, caseId) {
    try {
      const db = this.deps.db;
      const now = Date.now();
      // Query active subscriptions with webhook and agentAccess containing 'ModerationAgent'
      ...
      for (const sub of subscribers) {
        try {
          ...
          this.logger.debug(`✅ B2B moderation webhook delivered to ${sub.guildId} (${actionType})`);
        } catch (err) {
          this.logger.warn(`❌ B2B moderation webhook failed for ${sub.guildId}: ${err.message}`);
          // Optionally increment failure count – we can implement later
        }
      }
    } catch (err) {
      this.logger.error(`B2B delivery error: ${err.message}`);
    }
  }
...
- suggestedAction: "warn", "timeout", "kick", "ban", or "ignore"`;
  ...
  async _executeAction(message, action, reason, severity) {
    ...
    try {
      ...
      // 🆕 Deliver to B2B subscribers
      await this._deliverToB2B(embed, guildId, action, caseId);
  ...
  async applyMute(guildId, userId, durationMs, reason) {
    ...
    if (member?.moderatable) {
      ...
      await this._sendModLog(guildId, { embeds: [embed] });
      // 🆕 B2B delivery for manual mute (though not triggered via command? we'll handle in command)
    }
  }
  ...
  async autoModAction(message, reason) {
    try {
      ...
      // 🆕 B2B delivery for auto-mod
      const caseId = await this._createCase(message.guild.id, message.author.id, 'AutoMod', reason, reason, message.content);
      await this._deliverToB2B(logEmbed, message.guild.id, 'auto_mod', caseId);
  ...
  async activateRaidMode(guildId) {
    const embed = new EmbedBuilder()
      ...
      .setColor(0xff0000);
    await this._sendModLog(guildId, { embeds: [embed] });
    // 🆕 B2B delivery for raid mode
    await this._deliverToB2B(embed, guildId, 'raid_mode', null);
  ...
  // ---- Command implementations (with B2B delivery where applicable) ----
  async cmdWarn(interaction) {
    ...
    const caseId = await this._createCase(interaction.guild.id, target.id, interaction.user.id, 'warn', reason);
    const embed = new EmbedBuilder()
    ...
    await this._deliverToB2B(embed, interaction.guild.id, 'warn', caseId);
  ...
  async cmdMute(interaction) {
    ...
    const caseId = await this._createCase(interaction.guild.id, target.id, interaction.user.id, 'timeout', reason);
    const embed = new EmbedBuilder()
      .setTitle('🔇 User Muted')
      .setColor(0xffaa00)
      .addFields(
      ...
      .setTimestamp();
    await this._sendModLog(interaction.guild.id, { embeds: [embed] });
    await this._deliverToB2B(embed, interaction.guild.id, 'timeout', caseId);
  ...
  async cmdKick(interaction) {
    ...
    const caseId = await this._createCase(interaction.guild.id, target.id, interaction.user.id, 'kick', reason);
    const embed = new EmbedBuilder()
      ...
      .setTimestamp();
    await this._sendModLog(interaction.guild.id, { embeds: [embed] });
    await this._deliverToB2B(embed, interaction.guild.id, 'kick', caseId);
  ...
  async cmdBan(interaction) {
    ...
    const caseId = await this._createCase(interaction.guild.id, target.id, interaction.user.id, 'ban', reason);
    const embed = new EmbedBuilder()
      ...
      .setTimestamp();
    await this._sendModLog(interaction.guild.id, { embeds: [embed] });
    await this._deliverToB2B(embed, interaction.guild.id, 'ban', caseId);
  ...
  async cmdPurge(interaction) {
    ...
    // No B2B for purge (not a user action)
  ...
  async cmdAppeal(interaction) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setTimestamp();
    await this._sendModLog(interaction.guild.id, { embeds: [embed] });
    // 🆕 B2B: we might not need to send appeal to B2B subscribers, but we can if desired
    // await this._deliverToB2B(embed, interaction.guild.id, 'appeal', caseId);
